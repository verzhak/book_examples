...
#define _GNU_SOURCE		/* Для setresuid(), setresgid(), getresuid() и getresgid() */

#include <stdio.h>
#include <stdlib.h>

#include <unistd.h>		/* Для setresuid(), setresgid(), getresuid() и getresgid() */
...
int main()
{
	...
	if(
		 ...
		 * групп пользователей процесса случайные целые в диапазоне от 1 до 3 */
		setresgid(rand() % 3 + 1, rand() % 3 + 1, rand() % 3 + 1) == -1
		||
		/* Устанавливаем в качестве реального, действительного (эффективного) и сохраненного идентификаторов
		 * пользователей процесса случайные целые в диапазоне от 1 до 3 */
		setresuid(rand() % 3 + 1, rand() % 3 + 1, rand() % 3 + 1) == -1
	  )
